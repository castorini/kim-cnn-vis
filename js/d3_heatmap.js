function drawHeatmap(query, highlight) {
  $("#Matrix").empty();
  ...
  drawWeight(query.length-2, highlight[0]);
  drawWeight(query.length-3, highlight[1]);
  drawWeight(query.length-4, highlight[2]);
}

function drawWeight(len, input) {
  var data_values = [];
  var data_labels = [];
